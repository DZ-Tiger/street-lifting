...
export default function ProgressionView() {
  ...
  const filteredLogs = useMemo(() => {
    ...
    return exerciseLogs
      .filter(log => log.exercise_name === activeExercise && log.date && isAfter(new Date(log.date), threeMonthsAgo))
          ...
          <Card className="bg-white border-slate-100 rounded-3xl shadow-sm">
            <CardContent className="p-5">
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className="text-[9px] font-black uppercase text-slate-400 tracking-wider">Dernier 1RM</span>
              </div>
          ...
          <Card className="bg-white border-slate-100 rounded-3xl shadow-sm">
            <CardContent className="p-5">
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className="text-[9px] font-black uppercase text-slate-400 tracking-wider">Volume (Set)</span>
              </div>
              <p className="text-2xl font-black text-slate-800 italic">
        ...
        <div className="bg-white border border-slate-100 p-5 rounded-[2rem] shadow-sm">
          <div className="grid grid-rows-7 gap-1.5 grid-flow-col overflow-x-auto pb-2 scrollbar-hide">
            {Array.from({ length: 84 }).map((_, i) => {
              ...
              const sessionDates = new Set(completedSessions.map(s => format(new Date(s.date), 'yyyy-MM-dd')));
        ...
        <Card className="bg-white border-slate-100 rounded-[2rem] shadow-sm overflow-hidden relative">
          {!hasEnoughData && (
             <div className="absolute inset-0 z-10 bg-white/80 backdrop-blur-sm flex flex-col items-center justify-center p-6 text-center space-y-3">
               ...
               <p className="text-sm font-black text-slate-800 uppercase italic">Débloque tes prévisions</p>
               <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest leading-relaxed">
                 Continue à t&apos;entraîner ! Il nous faut au moins 3 séances de {activeExercise} pour calculer ta tendance.
               </p>
             </div>
          )}
                 ...
                 <ResponsiveContainer width="100%" height="100%">
                   <LineChart data={chartData} margin={{ top: 5, right: 10, left: -20, bottom: 0 }}>
                     ...
                     <Tooltip 
                       ...
                       labelStyle={{ color: '#64748b', marginBottom: '4px', textTransform: 'uppercase', fontSize: '10px', letterSpacing: '0.1em' }}
                     />
                     <Line 
                       type="monotone" 
                       dataKey="Tendance" 
        ...
        <div className="space-y-3">
          {prHistory.length > 0 ? (
            prHistory.map((pr, idx) => (
              <div key={idx} className="bg-white border border-slate-100 rounded-2xl p-4 flex items-center justify-between shadow-sm relative overflow-hidden group hover:border-emerald-200 transition-colors">
                {idx === 0 && (
                   <div className="absolute left-0 top-0 bottom-0 w-1 bg-emerald-500" />
                )}
                <div className="space-y-1 pl-2">
                   <div className="flex items-center gap-2">
                     <span className="font-black text-slate-900 uppercase italic text-xs">{pr.exercise_name}</span>
                     {idx === 0 && <Badge className="bg-emerald-50 text-emerald-600 border-none px-1.5 py-0 text-[8px] font-black uppercase tracking-widest">Actuel</Badge>}
                   </div>
